...
| Clearing | Each requested tile must be an observed `nearbyObjects` entry with Axe, Pickaxe, or Scythe recommended; stand beside it, select the matching tool, face it, and verify the object disappears while reporting inventory gains | Unproven until the live check; bounded per-target swing limits do not judge whether clearing the obstacle was strategically useful |
